 ...
 * my_malloc is aligned properly for the objects that are stored in it, all
...
void my_free(void *ptr) {
     ...
     * my_malloc because the next search in the iteration looks at exactly the
     * right memory block.
     */
    freep = currp;
}
...
void *calloc(unsigned long n, unsigned long size) {
    ...
    nb = n * size;
    fprintf(stderr, "calloc(): attempting to allocate %lu objects size %lu %lu bytes\n", n, size, nb);/*
#ifdef DEBUG
    fprintf(stderr, "calloc(): attempting to allocate %lu objects size %lu %lu bytes\n", n, size, nb);
#endif*/

    // Attempt to allocate the space and, if successful, clear it
    if ((q = my_malloc(nb)) != NULL)
        for (p = q, i = 0; i < nb; i++)
            *p++ = 0;
...
int main(void) {
    ...
    int nunits = (49 + sizeof(Header) - 1) / sizeof(Header) + 1;

    fprintf(stdout,"nunits=%d\n", nunits);
    fprintf(stdout,"sizeof header=%lu\n", sizeof(Header));
    fprintf(stdout,"49/16=%d\n\n", 49/16);

    memptr = my_malloc(1024);
    fprintf(stdout, "main(): memory allocated at %x\n", (unsigned int)memptr);
    if (memptr != NULL)
        my_free(memptr);
    memptr = calloc(5, sizeof(double));
    fprintf(stdout, "calloc: memory allocated at %x\n", (unsigned int)memptr);
    if (memptr != NULL)
        my_free(memptr);

    // This next allocation should push us over the limit
    memptr = calloc(2*1024, 1024);
    fprintf(stdout, "calloc: memory allocated at %x\n", (unsigned int)memptr);
    if (memptr != NULL)
        my_free(memptr);
}